...
const Activity = () => {
  ...
  return (
    <>
      ...
      }} /> : formData.accountnumber !== '' && searchActivity?.length === 0 ? (<Text style={{ textAlign: 'center' }}> No search results found</Text>) : (<FlatList data={searchActivity} renderItem={(item) => ActivityRenderFlatList(item)} keyExtractor={(item) => item._id}  style={{marginTop :30}} />)}
      {/* </MiddleCard> */}
      {/* </Card>  */}
